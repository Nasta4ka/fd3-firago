...
addToCart: (state, action) => {
    ...
    state.itemsInCart.map(product => product.id === itemToCart.id ? (product.quantity += itemToCart.quantity) : product)
...
deleteFromCart: (state, action) => {
    console.log(action)
    state.itemsInCart = state.itemsInCart.filter(product => product.id !== action.payload[0])
